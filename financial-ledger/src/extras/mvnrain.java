...
public class GitMavenAutomation {
    ...
    // Maven build with retry logic and rainbow progress simulation
    private static boolean buildMavenProjectWithRetry(File repoDir) {
        ...
        while (!buildSuccess && attempts < MAX_RETRIES) {
            ...
            buildSuccess = simulateRainbowProgress(repoDir.getName());
    ...
    // Simulate a rainbow progress bar
    private static boolean simulateRainbowProgress(String repoName) {
        int totalSteps = 100;
        System.out.print("Building " + repoName + " [");

        for (int i = 0; i <= totalSteps; i++) {
            String color = getRainbowColor(i, totalSteps);
            System.out.print("\u001B[" + color + "m#\u001B[0m"); // Print the progress symbol in color
            try {
                Thread.sleep(50); // Adjust this for faster/slower animation
            } catch (InterruptedException e) {
        ...
        return new Random().nextBoolean(); // Simulate random success for demonstration
    }

    // Get color code for rainbow effect based on progress
    private static String getRainbowColor(int step, int total) {
        float ratio = (float) step / total;
        int r = (int) (Math.sin(ratio * Math.PI * 2) * 127 + 128); // Red value
        int g = (int) (Math.sin((ratio * Math.PI * 2) + (Math.PI / 3)) * 127 + 128); // Green value
        int b = (int) (Math.sin((ratio * Math.PI * 2) + (Math.PI / 1.5)) * 127 + 128); // Blue value
        return "38;2;" + r + ";" + g + ";" + b; // RGB ANSI color code
    }
